...
### added for calculating delta
class XinvyJackknife(Jackknife):
    ...
    Parameters
    ----------
    est : float or np.array with shape (1, n)
        Whole data delta (X^(-1)@y) estimate
    X_delete_values : np.matrix with shape (n_blocks, n, n)
        Delete values for the X.
    y_delete_values: np.matrix with shape (n_blocks, n)
        Delete values for the y.

    Methods
    -------
    delete_vals_to_pseudovalues(est, X_delete_values, y_delete_values):
        Converts X, y delete values and the whole-data estimate to
        pseudovalues.

    '''

    def __init__(self, est, X_delete_values, y_delete_values):
        if X_delete_values.shape[0] != y_delete_values.shape[0]:
            raise ValueError(
                'X_delete_values.shape[0] != y_delete_values.shape[0].')
        if len(X_delete_values.shape) != 3 or X_delete_values.shape[1] != X_delete_values.shape[2]:
            raise ValueError('X delete values must be square matrices.')
        if est.shape != y_delete_values.shape[1:]:
            raise ValueError(
                'Shape of est does not match shape of delete values.')

        self.n_blocks = X_delete_values.shape[0]
        self.est = est
        self.pseudovalues = self.delete_values_to_pseudovalues(self.est,
                                                               X_delete_values, y_delete_values)
        (self.jknife_est, self.jknife_var, self.jknife_se, self.jknife_cov) =\
            self.jknife(self.pseudovalues)

    @classmethod
    def delete_values_to_pseudovalues(cls, est, X_delete_values, y_delete_values):
        '''
        Converts delete values to pseudovalues.

        Parameters
        ----------
        est : np.matrix with shape (1, n)
            Whole-data delta (X^(-1)@y) estimate.
        X_delete_values : np.matrix with shape (n_blocks, n, n)
            X delete values.
        y_delete_values : np.matrix with shape (n_blocks, n)
        ...
        -------
        pseudovalues :
            delta Jackknife Pseudovalues.

        Raises
        ------
        ValueError :
            If X_delete_values.shape != y_delete_values.shape.

        '''
        n_blocks, n = y_delete_values.shape
        pseudovalues = np.zeros((n_blocks, n))
        for j in range(0, n_blocks):
            pseudovalues[j] = np.dot(np.linalg.inv(X_delete_values[j]), y_delete_values[j])
        cls.jknife_ests = pseudovalues
        return pseudovalues