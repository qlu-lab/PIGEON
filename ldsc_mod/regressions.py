...
def gencov_obs_to_liab(gencov_obs, P1, P2, K1, K2):
    '''
    Converts  on the observed scale in an ascertained sample to genetic
    covariance on the liability scale in the population

    Parameters
    ----------
    gencov_obs : float
         on the observed scale in an ascertained sample.
    P1, P2 : float in (0,1)
    ...
    -------
    gencov_liab : float
         between liabilities in the population.
...
class Hsq(LD_Score_Regression):
    ...
    def __init__(self, y, x, w, N, M, n_blocks=200, intercept=None, slow=False, twostep=None, old_weights=False):
        ...
        if not self.constrain_intercept:
            ...
            self.intercept_p,self.intercept_z = p_z_norm(self.intercept, self.intercept_se)
    ...
    def format_output(self, ref_ld_colnames, P=None, K=None, gxe=None, gwas=None):
        '''Format summary of the LD Score regression as DataFrame.'''
        ...
        r = lambda x: np.array(x).reshape((len(ref_ld_colnames)))

        if gwas is not None:
            if gxe is not None: 
                df = pd.DataFrame({
                    'gxe_sumstats': [gxe] * len(ref_ld_colnames),
                    'gwas_sumstats': [gwas] * len(ref_ld_colnames),
                    'Category': ref_ld_colnames,
                    T + '_scale_h2': r(c * self.cat),
                    T + '_scale_h2_SE': r(c * self.cat_se),
                    'Proportion_of_SNPs': r(self.M_prop),
                    'Proportion_of_h2': r(self.prop),
                    'Enrichment': r(self.enrichment),
                    'Coefficient': r(self.coef),
                    'Coefficient_SE': r(self.coef_se)
                })[['gxe_sumstats', 'gwas_sumstats', 'Category', T + '_scale_h2', T + '_scale_h2_SE', 'Proportion_of_SNPs', 'Proportion_of_h2', 'Enrichment', 'Coefficient', 'Coefficient_SE']]
            else:
                df = pd.DataFrame({
                    'gwas_sumstats': [gwas] * len(ref_ld_colnames),
                    'Category': ref_ld_colnames,
                    ...
                    'Coefficient': r(self.coef),
                    'Coefficient_SE': r(self.coef_se)
                })[['gwas_sumstats', 'Category', T + '_scale_h2', T + '_scale_h2_SE', 'Proportion_of_SNPs', 'Proportion_of_h2', 'Enrichment', 'Coefficient', 'Coefficient_SE']]
        else:
            df = pd.DataFrame({
                'Category': ref_ld_colnames,
                T + '_scale_h2': r(c * self.cat),
                ...
                'Coefficient': r(self.coef),
                'Coefficient_SE': r(self.coef_se)
            })[['Category', T + '_scale_h2', T + '_scale_h2_SE', 'Proportion_of_SNPs', 'Proportion_of_h2', 'Enrichment', 'Coefficient', 'Coefficient_SE']]
        return df
    ...
    def summary(self, ref_ld_colnames=None, P=None, K=None, overlap=False):
        ...
        if self.n_annot > 1:
            ...
            if not overlap:
                ...
                out.append('Proportion of h2: ' + s(self.prop))
...
class GxE_var(LD_Score_Regression):

    __null_intercept__ = 1

    def __init__(self, y, x, w, N, M, n_blocks=200, intercept=None, slow=False, twostep=None, old_weights=False, covariates_R2=0):
        step1_ii = None
        if twostep is not None:
            step1_ii = y < twostep
        y *= (1-covariates_R2)
        ...
        if not self.constrain_intercept:
            self.ratio, self.ratio_se = self._ratio(
                self.intercept, self.intercept_se, self.mean_chisq)
            self.intercept_p,self.intercept_z = p_z_norm(self.intercept, self.intercept_se)
        self.intercept /= (1-covariates_R2)
        self.intercept_se /= (1-covariates_R2)
    ...
    def summary(self, ref_ld_colnames=None, P=None, K=None, overlap=False):
        ...
        out = [ T + ' scale GxE variance component: ' +
               s(c * self.tot) + ' (' + s(c * self.tot_se) + ')']
        out.append('Z-score: ' + s(self.z))
        out.append('P: ' + s(self.p))
        if self.n_annot > 1:
...
class Gencov(LD_Score_Regression):
    ...
    def __init__(self, z1, z2, x, w, N1, N2, M, hsq1, hsq2, intercept_hsq1, intercept_hsq2,
        ...
        if self.n_annot > 1:
            self.cat_p = np.zeros(len(self.cat.flatten()))
            self.cat_z = np.zeros(len(self.cat.flatten()))
            for i in range(len(self.cat.flatten())):
                self.cat_p[i], self.cat_z[i] = p_z_norm(self.cat.flatten()[i], self.cat_se.flatten()[i])

            enrichment_test = np.divide(self.cat, M) - np.divide(self.tot - self.cat, float(np.sum(M)) - M)
            enrichment_test_se = np.std(enrichment_test.flatten())
            self.enrichment_p = np.zeros(len(self.cat.flatten()))
            self.enrichment_z = np.zeros(len(self.cat.flatten()))
            for i in range(len(self.cat.flatten())):
                self.enrichment_p[i], self.enrichment_z[i] = p_z_norm(enrichment_test.flatten()[i], enrichment_test_se)
        ...
        if gwas is not None:
            if gxe is not None: 
                df = pd.DataFrame({
                    ...
                    'gwas_sumstats': [gwas] * len(ref_ld_colnames),
                    'Category': ref_ld_colnames,
                    T + '_scale_gencov': r(c * self.cat),
                    T + '_scale_gencov_SE': r(c * self.cat_se),
                    T + '_scale_gencov_P': r(self.cat_p),
                    'Proportion_of_SNPs': r(self.M_prop),
                    'Proportion_of_gencov': r(self.prop),
                    'Enrichment': r(self.enrichment),
                    'Enrichment_P': r(self.enrichment_p)
                })[['gxe_sumstats', 'gwas_sumstats', 'Category', T + '_scale_gencov', T + '_scale_gencov_SE', T + '_scale_gencov_P', 'Proportion_of_SNPs', 'Proportion_of_gencov', 'Enrichment', 'Enrichment_P']]
            else:
                df = pd.DataFrame({
                    'gwas_sumstats': [gwas] * len(ref_ld_colnames),
                    'Category': ref_ld_colnames,
                    ...
                    'Enrichment': r(self.enrichment),
                    'Enrichment_P': r(self.enrichment_p)
                })[['gwas_sumstats', 'Category', T + '_scale_gencov', T + '_scale_gencov_SE', T + '_scale_gencov_P', 'Proportion_of_SNPs', 'Proportion_of_gencov', 'Enrichment', 'Enrichment_P']]
        else:
            df = pd.DataFrame({
                'Category': ref_ld_colnames,
                T + '_scale_gencov': r(c * self.cat),
                ...
                'Enrichment': r(self.enrichment),
                'Enrichment_P': r(self.enrichment_p)
            })[['Category', T + '_scale_gencov', T + '_scale_gencov_SE', T + '_scale_gencov_P', 'Proportion_of_SNPs', 'Proportion_of_gencov', 'Enrichment', 'Enrichment_P']]
        df = df.sort_values(by=[T + '_scale_gencov_P'])
        return df
    ...
    def summary(self, ref_ld_colnames, P=None, K=None):
        ...
        # out.append( T + ' scale covgxe: ' +
        #            s(c * self.tot) + ' (' + s(c * self.tot_se) + ')')
        out.append(
                 T + ' scale covariant GxE: ' + s(c * self.tot) +
                ' (' + s(c * self.tot_se) + ')')
        out.append('Z-score: ' + s(self.z))
        out.append('P: ' + s(self.p))
        if self.n_annot > 1:
            ...
            out.append(T + ' scale gencov P: ' + s(self.cat_p))
            out.append('Proportion of SNPs: ' + s(self.M_prop))
            out.append('Proportion of gencov: ' + s(self.prop))
            out.append('Enrichment: ' + s(self.enrichment))
            out.append('Enrichment P: ' + s(self.enrichment_p))
...
class RG(object):
    ...
    def __init__(self, z1, z2, x, w, N1, N2, M, intercept_hsq1=None, intercept_hsq2=None,
        ...
        if (hsq1.tot <= 0 or hsq2.tot <= 0):
            ...
            self.rg_ratio = self.rg = self.rg_se = np.nan
            ...
            # add rg_ratio_delete_values here
            self.rg_ratio_delete_values = np.array([gencov.tot_delete_values[blk]/denom_delete_values[blk] for blk in range(n_blocks)])
...
class oPGSxE_scan(object):

    def __init__(self, z1, z2, x, w, N1, N2, M, intercept_hsq1=None, intercept_hsq2=None,
                 intercept_gencov=None, n_blocks=200, slow=False, twostep=None, e_sd=1, covariates_R2 = 0):
        self.intercept_gencov = intercept_gencov
        self._negative_hsq = None
        n_snp, self.n_annot = x.shape

        # Adjust for covariates R2
        z1 = z1 * np.sqrt(1-covariates_R2)
        # z2 = z2 * np.sqrt(1-covariates_R2)

        hsq1 = Hsq(np.square(z1), x, w, N1, M, n_blocks=n_blocks, intercept=intercept_hsq1, # GxE heritability
                   slow=slow, twostep=twostep)
        hsq2 = Hsq(np.square(z2), x, w, N2, M, n_blocks=n_blocks, intercept=intercept_hsq2, # additive heritability
                   slow=slow, twostep=twostep)
        gencov = Gencov(z1, z2, x, w, N1, N2, M, hsq1.tot, hsq2.tot, hsq1.intercept,  # covariant GxE
                        hsq2.intercept, n_blocks, intercept_gencov=intercept_gencov, slow=slow,
                        twostep=twostep)
        gencov.N1 = None  # save memory
        gencov.N2 = None
        ...
        self.hsq1, self.hsq2, self.gencov = hsq1, hsq2, gencov
        if (hsq2.tot <= 0): # change
            self._negative_hsq = True
            self.rg_ratio = self.opgsxe = self.rg_se = np.nan
            self.p = self.z = np.nan
        else:
            rg_ratio = np.array(
                gencov.tot / np.sqrt(hsq2.tot)).reshape((1, 1))
            hsq2.tot_delete_values = np.array([[h2 if h2>0 else np.nan for h2 in hsq2.tot_delete_values[i]] for i in range(len(hsq2.tot_delete_values))]) # added
            denom_delete_values = np.sqrt(hsq2.tot_delete_values)
            opgsxe = jk.RatioJackknife(
                rg_ratio, gencov.tot_delete_values, denom_delete_values)
            # add rg_ratio_delete_values here
            self.rg_ratio_delete_values = np.array([gencov.tot_delete_values[blk]/denom_delete_values[blk] for blk in range(n_blocks)])
            self.rg_jknife = float(opgsxe.jknife_est)
            self.rg_se = float(opgsxe.jknife_se) # add environmental sd here
            self.rg_ratio = float(rg_ratio)
            self.p, self.z = p_z_norm(self.rg_ratio, self.rg_se)
            
            # Adjust for environmental sd here
            self.rg_ratio /= e_sd
            self.rg_se /= e_sd
            self.rg_ratio_delete_values /= e_sd

            if self.n_annot > 1:
                # per category
                hsq2.cat = [[h2 if h2>0 else np.nan for h2 in hsq2.cat[0]]]
                cat_rg_ratio_list = np.array(gencov.cat[0] / np.sqrt(hsq2.cat[0]))
                hsq2.part_delete_values = np.array([[h2 if h2>0 else np.nan for h2 in hsq2.part_delete_values[i]] for i in range(len(hsq2.part_delete_values))])
                cat_rg_se_list = []
                for c, cat_rg_ratio in enumerate(cat_rg_ratio_list):
                    cat_denom_delete_values = np.sqrt(np.array(hsq2.part_delete_values[:, c]*M[0][c]).reshape((n_blocks, 1)))
                    cat_opgsxe = jk.RatioJackknife(
                        np.array([[cat_rg_ratio]]), np.array(gencov.part_delete_values[:, c]*M[0][c]).reshape((n_blocks, 1)), cat_denom_delete_values)
                    cat_rg_se_list.append(float(cat_opgsxe.jknife_se[0, 0]))
                self.cat_rg_ratio = cat_rg_ratio_list
                self.cat_rg_se = np.array(cat_rg_se_list)
                self.cat_p = np.zeros(len(cat_rg_ratio_list.flatten()))
                self.cat_z = np.zeros(len(cat_rg_ratio_list.flatten()))
                for i in range(len(cat_rg_ratio_list.flatten())):
                    self.cat_p[i], self.cat_z[i] = p_z_norm(self.cat_rg_ratio.flatten()[i], self.cat_rg_se.flatten()[i])

                # Adjust for environmental sd here
                self.cat_rg_ratio /= e_sd
                self.cat_rg_se /= e_sd

    def format_output(self, ref_ld_colnames, silly=False, gxe=None, gwas=None):
        '''Format summary of the LD Score regression as DataFrame.'''
            ...
            if gwas is not None:
                if gxe is not None: 
                    df = pd.DataFrame({
                        ...
                        'gwas_sumstats': [gwas] * len(ref_ld_colnames),
                        'Category': ref_ld_colnames,
                        'PGSxE': r(self.cat_rg_ratio),
                        'PGSxE_SE': r(self.cat_rg_se),
                        'P': self.cat_p
                    })[['gxe_sumstats', 'gwas_sumstats', 'Category', 'PGSxE', 'PGSxE_SE', 'P']]
                else:
                    df = pd.DataFrame({
                        'gwas_sumstats': [gwas] * len(ref_ld_colnames),
                        'Category': ref_ld_colnames,
                        ...
                        'PGSxE_SE': r(self.cat_rg_se),
                        'P': self.cat_p
                    })[['gwas_sumstats', 'Category', 'PGSxE', 'PGSxE_SE', 'P']]
            else:
                df = pd.DataFrame({
                    'Category': ref_ld_colnames,
                    'PGSxE': r(self.cat_rg_ratio),
                    'PGSxE_SE': r(self.cat_rg_se),
                    'P': self.cat_p
                })[['Category', 'PGSxE', 'PGSxE_SE', 'P']]
            df = df.sort_values(by=['P'])
        return df

    def summary(self, ref_ld_colnames, silly=False):
        '''Print output of Gencor object.'''
        out = []
        if self._negative_hsq:
            out.append('Oracle PGSxE: nan (nan) (h2  out of bounds) ')
            out.append('Z-score: nan (nan) (h2  out of bounds)')
            out.append('P: nan (nan) (h2  out of bounds)')
            out.append('WARNING: One of the h2\'s was out of bounds.')
            out.append(
            ...
            out.append('Oracle PGSxE: nan (nan) (opgsxe out of bounds) ')
            out.append('Z-score: nan (nan) (opgsxe out of bounds)')
            out.append('P: nan (nan) (opgsxe out of bounds)')
            out.append('WARNING: opgsxe was out of bounds.')
            if self.intercept_gencov is None:
                out.append(
                    'This often means that h2 is not significantly ' +
                    'different from zero.')
            ...
            if self.n_annot > 1:
                out.append('Categories: ' + str(' '.join(ref_ld_colnames)))
                out.append('PGSxE: ' + s(self.cat_rg_ratio))
                out.append('PGSxE SE: ' + s(self.cat_rg_se))
                out.append('P: ' + s(self.cat_p) )
        return remove_brackets('\n'.join(out))

class oPGSxe_scan(object):

    def __init__(self, Z_list, x, w, N_list, M, intercepts_hsq=None, intercepts_gencov=None, n_blocks=200, slow=False, twostep=None, e_sd=1, covariates_R2=0):
        self._negative_hsq = None

        # Adjust for covariates R2
        Z_list[0] *=  np.sqrt(1-covariates_R2)

        hsq_list = [Hsq(np.square(Z_list[i]), x, w, N_list[i], M, n_blocks=n_blocks, intercept=intercepts_hsq[i] if len(intercepts_hsq)>i else None, slow=slow, twostep=twostep) for i in range(len(Z_list))]
        combinations = [(i,j) for i in range(len(Z_list)) for j in range(i+1, len(Z_list))] # it matters when mentioning the order of gencov
        gencov_list = [Gencov(Z_list[c[0]], Z_list[c[1]], x, w, N_list[c[0]], N_list[c[1]], M, hsq_list[c[0]].tot, hsq_list[c[1]].tot, hsq_list[c[0]].intercept, hsq_list[c[1]].intercept, n_blocks, intercept_gencov=intercepts_gencov[i] if len(intercepts_gencov)>i else None, slow=slow, twostep=twostep) for i, c in enumerate(combinations)]
        
        if intercepts_gencov[0] == 0: # args.no_intercept == True
            gencov_list[0] = Gencov(Z_list[0], Z_list[1], x, w, N_list[0], N_list[1], M, hsq_list[0].tot, hsq_list[1].tot, hsq_list[0].intercept, hsq_list[1].intercept, n_blocks, intercept_gencov=None, slow=slow, twostep=twostep)

        for c in range(len(combinations)):
            gencov_list[c].N1 = None
            gencov_list[c].N2 = None

        self.hsq_list, self.gencov_list, self.covorder = hsq_list, gencov_list, combinations
        self.hsq_E, self.hsq_add = self.hsq_list[1], self.hsq_list[2:]
        self.covar_gxe_e = self.gencov_list[0].tot / (1 + self.hsq_E.tot)
        self.covar_gxe_e_delete_values = self.gencov_list[0].tot_delete_values / (1 + self.hsq_E.tot_delete_values)
        # self.covar_gxe_e = np.mean(self.covar_gxe_e_delete_values)
        self.gencov, self.gencov_g_e = self.gencov_list[1:(len(Z_list)-1)], self.gencov_list[(len(Z_list)-1):(2*len(Z_list)-3)]

        # hsq GxE
        self.h2_gxe = np.array(hsq_list[0].tot - 2 * self.covar_gxe_e ** 2).reshape((1,1))
        self.h2_gxe_delete_values = np.array(hsq_list[0].tot_delete_values - 2 * self.covar_gxe_e_delete_values ** 2)
        h2gxe = jk.RatioJackknife(self.h2_gxe, self.h2_gxe_delete_values, np.array([1] * n_blocks).reshape((n_blocks, 1)))
        self.h2_jknife = float(h2gxe.jknife_est)
        self.h2_se = float(h2gxe.jknife_se)
        self.h2_p, self.h2_z = p_z_norm(self.h2_gxe, self.h2_se)

        # Covariant GxE
        self.covar_gxe = [gencov_list[i].tot - self.covar_gxe_e * gencov_list[i+len(Z_list)-2].tot for i in range(1, len(Z_list)-1)]
        self.covar_gxe_delete_values = [gencov_list[i].tot_delete_values - self.covar_gxe_e_delete_values * gencov_list[i+len(Z_list)-2].tot_delete_values for i in range(1, len(Z_list)-1)]
        # # Use the bias-corrected jackknife estimate
        # self.covar_gxe=[np.mean(np.array(self.covar_gxe_delete_values[i])) for i in range(0, len(Z_list)-2)]
        # print(self.covar_gxe)
        self.covar_jknife = []
        self.covar_se = []
        self.covar_p = []
        self.covar_z = []
        for i in range(len(Z_list)-2):
            covgxe = jk.RatioJackknife(np.array(self.covar_gxe[i]).reshape((1,1)), np.array(self.covar_gxe_delete_values[i]), np.array([1] * n_blocks).reshape((n_blocks, 1)))
            self.covar_jknife.append(float(covgxe.jknife_est))
            self.covar_se.append(float(covgxe.jknife_se))
            covar_p, covar_z = p_z_norm(self.covar_gxe[i], self.covar_se[i])
            self.covar_p.append(covar_p)
            self.covar_z.append(covar_z)
        ...
        self.z = []
        for i in range(len(Z_list)-2):
            if (hsq_list[i+2].tot <= 0):
                self._negative_hsq[i] = True
                self.rg_ratio.append(np.nan)
                self.rg_jknife.append(np.nan)
                self.rg_se.append(np.nan)
                self.p.append(np.nan)
                self.z.append(np.nan)
            else:
                nom_values = self.covar_gxe[i]
                denom_values = np.sqrt(hsq_list[i+2].tot)
                rg_ratio = np.array(nom_values / denom_values).reshape((1, 1))
                nom_delete_values = np.array(self.covar_gxe_delete_values[i])
                denom_delete_values = np.array(np.sqrt(hsq_list[i+2].tot_delete_values))
                opgsxe = jk.RatioJackknife(rg_ratio, nom_delete_values, denom_delete_values)
                self.rg_jknife.append(float(opgsxe.jknife_est))
                self.rg_se.append(float(opgsxe.jknife_se / e_sd )) # add environmental sd here
                self.rg_ratio.append(float(rg_ratio / e_sd))
                p, z = p_z_norm(self.rg_ratio[i], self.rg_se[i])
                self.p.append(p)
                self.z.append(z)
...
class GxE_var_e(object):

    def __init__(self, z1, z2, x, w, N1, N2, M, intercept_hsq1=None, intercept_hsq2=None,
                 intercept_gencov=None, n_blocks=200, slow=False, twostep=None, covariates_R2=0):
        # Adjust for covariates R2
        z1 *= np.sqrt(1-covariates_R2)
        # z2 = z2 * np.sqrt(1-covariates_R2)
        self.intercept_gencov = intercept_gencov
        self.constrain_intercept = intercept_hsq1 is not None
        hsq1 = Hsq(np.square(z1), x, w, N1, M, n_blocks=n_blocks, intercept=intercept_hsq1,
                   slow=slow, twostep=twostep)
        hsq2 = Hsq(np.square(z2), x, w, N2, M, n_blocks=n_blocks, intercept=intercept_hsq2,
                   slow=slow, twostep=twostep)
        ...
        # Covariant GxE between E additive and GxE
        self.covar_gxe_e =gencov.tot / (1 + self.hsq2.tot)
        self.covar_gxe_e_delete_values = gencov.tot_delete_values / (1 + self.hsq2.tot_delete_values)
        # hsq GxE
        self.h2_gxe = np.array(hsq1.tot - 2 * self.covar_gxe_e ** 2).reshape((1,1))
        self.h2_gxe_delete_values = np.array(hsq1.tot_delete_values - 2 * gencov.tot_delete_values ** 2)
        h2gxe = jk.RatioJackknife(self.h2_gxe, self.h2_gxe_delete_values, np.array([1] * n_blocks).reshape((n_blocks, 1)))
        self.h2_jknife = float(h2gxe.jknife_est)
        self.h2_se = float(h2gxe.jknife_se)
        self.h2_p, self.h2_z = p_z_norm(self.h2_gxe, self.h2_se)

        # self.h2_intercept = np.array(hsq1.intercept - gencov.intercept ** 2).reshape((1,1))
        # self.h2_intercept_delete_values = np.array(hsq1.intercept_delete_values - gencov.intercept_delete_values ** 2).reshape((n_blocks, 1))
        # h2intercept = jk.RatioJackknife(self.h2_intercept, self.h2_intercept_delete_values, np.array([1] * n_blocks).reshape((n_blocks, 1)))
        # self.h2_intercept_jknife = float(h2intercept.jknife_est)
        # self.h2_intercept_se = float(h2intercept.jknife_se)
        # self.h2_intercept_p, self.h2_intercept_z = p_z_norm(self.h2_intercept, self.h2_intercept_se)

        self.h2_gxe = self.h2_gxe[0][0]
        self.h2_p = self.h2_p[0][0]
        self.h2_z = self.h2_z[0][0]
        # self.h2_intercept = self.h2_intercept[0][0]
        # self.h2_intercept_p = self.h2_intercept_p[0][0]
        # self.h2_intercept_z = self.h2_intercept_z[0][0]
    ...
    def summary(self, ref_ld_colnames=None, P=None, K=None):
        ...
        out = [ T + ' scale GxE variance component: ' +
               s(c * self.h2_gxe) + ' (' + s(c * self.h2_se) + ')']
        out.append('Z-score: ' + s(self.h2_z))
        out.append('P: ' + s(self.h2_p))

        # if self.constrain_intercept:
        #     out.append(
        #         'Intercept: constrained to {C}'.format(C=s(self.h2_intercept)))
        # else:
        #     out.append(
        #         'Intercept: ' + s(self.h2_intercept) + ' (' + s(self.h2_intercept_se) + ')')

        return remove_brackets('\n'.join(out))

# cond analysis using nonstd PGS
class oPGSxE_cond_nonstd(object):
    ...
    def __init__(self, Z_list, x, w, N_list, M, intercepts_hsq=None, intercepts_gencov=None, n_blocks=200, slow=False, twostep=None, e_sd=1, covariates_R2=0):
        ...
        X = np.diag(list(map(lambda x: x.tot, hsq_list[1:])))
        y = np.array(list(map(lambda x: x.tot, gencov_list[:len(Z_list)-1])))
        for i, c in enumerate(combinations):
            if c[0] > 0:
                X[c[0]-1, c[1]-1] = gencov_list[i].tot
                X[c[1]-1, c[0]-1] = gencov_list[i].tot
        coeff_delta = np.dot(np.linalg.inv(X), y)
        self.coeff_delta = coeff_delta

        X_delete_values = np.array([np.diag(list(map(lambda x: x.tot_delete_values[blk][0], hsq_list[1:]))) for blk in range(n_blocks)])
        y_delete_values = np.array([np.array(list(map(lambda x: x.tot_delete_values[blk][0], gencov_list[:len(Z_list)-1]))) for blk in range(n_blocks)])
        for blk in range(n_blocks):
            for i, c in enumerate(combinations):
                if c[0] > 0:
                    X_delete_values[blk][c[0]-1, c[1]-1] = gencov_list[i].tot_delete_values[blk][0]
                    X_delete_values[blk][c[1]-1, c[0]-1] = gencov_list[i].tot_delete_values[blk][0]


        opgsxe = jk.XinvyJackknife(coeff_delta, X_delete_values, y_delete_values)
        self.coeff_delta_jknife = opgsxe.jknife_est
        self.coeff_delta_se = np.sqrt((n_blocks-1)/n_blocks*sum((opgsxe.jknife_ests-opgsxe.jknife_est)**2))
        self.coeff_delta_p,self.coeff_delta_z = p_z_norm(self.coeff_delta, self.coeff_delta_se)
        
        # oPGSxE ratio part
        hsq2_tot=list(map(lambda x: x.tot, hsq_list[1:]))
        hsq2_tot_delete_values=[list(map(lambda x: x.tot_delete_values[blk][0], hsq_list[1:])) for blk in range(n_blocks)] # change to match rule of blocks
        rg_ratio = np.array(
            self.coeff_delta / np.sqrt(hsq2_tot)).reshape((1, len(self.coeff_delta)))
        nom_delete_values = opgsxe.jknife_ests
        denom_delete_values = np.sqrt(hsq2_tot_delete_values)
        opgsxe = jk.RatioJackknife(
            rg_ratio, nom_delete_values, denom_delete_values)
        self.rg_jknife = opgsxe.jknife_est if len(self.coeff_delta)>1 else float(opgsxe.jknife_est)
        self.rg_se = opgsxe.jknife_se/e_sd if len(self.coeff_delta)>1 else float(opgsxe.jknife_se / e_sd) # add environmental sd here
        self.rg_ratio = rg_ratio/e_sd if len(self.coeff_delta)>1 else float(rg_ratio / e_sd)
        self.p, self.z = p_z_norm(self.rg_ratio, self.rg_se)
    ...
    def summary(self, silly=False):
        ...
        if self._negative_hsq:
            ...
            out.append('---------------------')
            out.append(
                'Oracle PGSxe: ' + s(self.rg_gxe_ratio) +
                ' (' + s(self.rg_gxe_se) + ')')
            out.append('Z-score: ' + s(self.z_gxe))
            out.append('P: ' + s(self.p_gxe))
        return remove_brackets('\n'.join(out))

# cond analysis using std PGS
class oPGSxE_cond(object):
    ...
    def __init__(self, Z_list, x, w, N_list, M, intercepts_hsq=None, intercepts_gencov=None, n_blocks=200, slow=False, twostep=None, e_sd=1, covariates_R2=0):
        ...
        oPGSxE_list = [oPGSxE_scan(Z_list[0], Z_list[i], x, w, N_list[0], N_list[i], M, intercept_hsq1=intercepts_hsq[0] if len(intercepts_hsq)>0 else None, intercept_hsq2=intercepts_hsq[i] if len(intercepts_hsq)>i else None, intercept_gencov=intercepts_gencov[i-1] if len(intercepts_gencov)>i-1 else None, n_blocks=n_blocks, slow=slow, twostep=twostep, e_sd=e_sd, covariates_R2=covariates_R2) for i in range(1, len(Z_list))]
        combinations = [(i, j) for i in range(len(Z_list)) for j in range(i+1, len(Z_list))] # it matters when mentioning the order of gencov
        r_list = [RG(Z_list[c[0]], Z_list[c[1]], x, w, N_list[c[0]], N_list[c[1]], M, intercept_hsq1=intercepts_hsq[c[0]] if len(intercepts_hsq)>c[0] else None, intercept_hsq2=intercepts_hsq[c[1]] if len(intercepts_hsq)>c[1] else None, intercept_gencov=intercepts_gencov[i] if len(intercepts_gencov)>i else None, n_blocks=n_blocks, slow=slow, twostep=twostep) for i, c in enumerate(combinations)]

        for c in range(len(combinations)):
            r_list[c].N1 = None
            r_list[c].N2 = None

        self.oPGSxE_list, self.r_list, self.covorder = oPGSxE_list, r_list, combinations
        
        # Covarince matrix beteen GxE and GWAS traits
        X = np.diag([float(1)]*(len(Z_list)-1))
        y = np.array(list(map(lambda x: x.rg_ratio, oPGSxE_list)))
        for i, c in enumerate(combinations):
            if c[0] > 0:
                X[c[0]-1, c[1]-1] = r_list[i].rg_ratio
                X[c[1]-1, c[0]-1] = r_list[i].rg_ratio
        coeff_delta = np.dot(np.linalg.inv(X), y)
        self.coeff_delta = coeff_delta

        X_delete_values = np.array([np.diag([float(1)]*(len(Z_list)-1)) for blk in range(n_blocks)])
        y_delete_values = np.array([np.array(list(map(lambda x: x.rg_ratio_delete_values[blk][0], oPGSxE_list))) for blk in range(n_blocks)])
        for blk in range(n_blocks):
            for i, c in enumerate(combinations):
                if c[0] > 0:
                    X_delete_values[blk][c[0]-1, c[1]-1] = r_list[i].rg_ratio_delete_values[blk][0]
                    X_delete_values[blk][c[1]-1, c[0]-1] = r_list[i].rg_ratio_delete_values[blk][0]
    ...
    ## summary need change considering output of opgsxe
    def summary(self, silly=False):
        '''Print output of Gencor object.'''
        out = []
        if self._negative_hsq:
